...
router.get('/:room', function(req, res) {
  ...
  Chatroom.findById(req.params.room, function(err, room) {
    ...
    //res.render('message', {user: req.user,room: room});
    if (room.secret){
        res.render('private', {title:'Chatroom 8', user: req.user,room: room});
    }
    else {
        res.render('message', {title:'Chatroom 8', user: req.user,room: room});
    }
